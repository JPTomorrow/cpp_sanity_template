...
static void update(Player *players, int players_len, Uint64 dt_ns)
{
    ...
    for (i = 0; i < players_len; i++)
    {
        ...
        if (player->pos[0] != posX)
            player->vel[0] = 0;
        if (player->pos[1] != posY)
            player->vel[1] = (wasd & 16) ? 8.4375 : 0;
        if (player->pos[2] != posZ)
            player->vel[2] = 0;
        player->pos[0] = posX;
        player->pos[1] = posY;
        player->pos[2] = posZ;
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[])
{
    ...
    std::cout << test_add(1, 2) << std::endl;
    std::cout << test_add(2, 4) << std::endl;
...
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event)
{
    ...
    case SDL_EVENT_KEY_DOWN:
    {
        ...
        {
            if (sym == SDLK_W)
                players[index].wasd |= 1;
            if (sym == SDLK_A)
                players[index].wasd |= 2;
            if (sym == SDLK_S)
                players[index].wasd |= 4;
            if (sym == SDLK_D)
                players[index].wasd |= 8;
            if (sym == SDLK_SPACE)
                players[index].wasd |= 16;
        }
        else if (id)
        {
            for (i = 0; i < MAX_PLAYER_COUNT; i++)
    ...
    case SDL_EVENT_KEY_UP:
    {
        ...
        {
            if (sym == SDLK_W)
                players[index].wasd &= 30;
            if (sym == SDLK_A)
                players[index].wasd &= 29;
            if (sym == SDLK_S)
                players[index].wasd &= 27;
            if (sym == SDLK_D)
                players[index].wasd &= 23;
            if (sym == SDLK_SPACE)
                players[index].wasd &= 15;
        }
        break;
    }
    }
